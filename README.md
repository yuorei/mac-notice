...
# 自分の音声ファイルを使う（mp3 / m4a 等は自動変換、30秒以内）
mac-notice --title "完了" --sound ~/Music/pikon.wav
mac-notice --title "完了" --sound ~/Music/pikon.mp3
...
`--sound` にパスを渡すと、ファイルを `~/Library/Sounds/mac-notice-<ファイル名>` にコピーしてから再生する（`UNNotificationSound` がパスを直接受け取れないため）。

- aiff / aif / wav / caf → そのままコピー
- mp3 / m4a / aac / flac 等 → `afconvert`（macOS標準）で caf に自動変換
- 30秒以内という制約あり（macOSの通知音の仕様）
- 変換失敗やファイルが見つからない場合は警告を出してデフォルト音にフォールバック
